...
void TelemetryImpl::enable()
{
    ...
    if (_parent->has_autopilot()) {
        ...
        std::vector<std::string> setupParams = { "INS_ACCOFFS_X", "INS_ACCOFFS_Y", "INS_ACCOFFS_Z" };
        for(const auto& param : setupParams)
        {
            _parent->get_param_float_async(
                param,
                std::bind(
                    [&](MAVLinkParameters::Result result, float value)
                    {
